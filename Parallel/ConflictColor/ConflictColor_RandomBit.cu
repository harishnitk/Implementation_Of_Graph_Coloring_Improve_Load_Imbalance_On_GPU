...
modified on 4-may-2019, first fit
*/
#include<bits/stdc++.h>
#include<cuda.h>
...
__device__ long int findUnsetRandomColor(long int Id,unsigned int *Colorset,long int maxClr){
    ...
    long int j,phase,end=(maxClr/32)+1,bit=0,count;
    
    for(phase=0;phase<end;phase++){
      
      count = __popc(Colorset[Id*end+phase]);
      count = Id%count;
      j = 0;
      while(count>=0&&Colorset[Id*end+phase]>0){
          if(Colorset[Id*end+phase]%2==1){
             bit = j+1;
             count--;
          }
          j++;
